"""Embed catalog reference images into wine_embeddings (pgvector).

Usage: python ml/scripts/build_index.py [--model NAME] [--batch 16] [--force]
"""
...
def main() -> None:
    ...
    with connect() as conn:
        done = set() if args.force else indexed_slugs(conn, args.model)
        todo = [w for w in wines if w["slug"] not in done]
        print(f"{len(wines)} wines, {len(done)} already indexed, {len(todo)} to embed")
        if not todo:
            return
        ...
        for i in tqdm(range(0, len(todo), args.batch), unit="batch"):
            ...
            upsert_embeddings(conn, args.model, [w["slug"] for w in chunk], vecs)
        dt = time.perf_counter() - t0
        print(f"indexed {len(todo)} in {dt:.0f}s ({1000 * dt / len(todo):.0f} ms/img), dim={emb.dim}")
