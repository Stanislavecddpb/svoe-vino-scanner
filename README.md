...
из 2103 slug в CSV — **2076 в индексе**. Как найдено фото — поле `mapping` в `wines.jsonl`:
...
`newest_reupload` 19 (одна бутылка загружена несколько раз — берётся новейший файл); `shared_photo` 22 (одно фото на несколько вин);
`fuzzy` 1. Имена сравниваются с учётом разных транслитераций Strapi (`belyj`/`belyy`, `Lvicza`, `ovoshhi`).
Исключено 27 (15 — файл не найден, 12 — несколько непохожих снимков бутылок / нет снимка бутылки), список — `data/catalog/excluded.csv`.

**Двойники** (`ml/scripts/find_twins.py` → `data/catalog/twins.csv`): 93 вина в 46 группах, у которых эталонное фото совпадает с фото другого вина (косинус ≥ 0.99) — одно изображение на разные вина или одно вино, заведённое дважды. По изображению они неразличимы: остаются в индексе, но в оценке считаются отдельно.
...
- 27 позиций каталога не сопоставлены с фото (см. `excluded.csv`); 93 вина — «двойники» с одинаковым эталоном.
